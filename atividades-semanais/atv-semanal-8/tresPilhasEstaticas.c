...
/* Inicialização da PILHATRIPLA (a PILHATRIPLA jah esta criada e eh apontada 
pelo endereco em p) */
...
/* Desloca a pilha 2 uma posição à esquerda */
...
/* Desloca a pilha 2 uma posição à direita */
...
/* excluirElementoPilha - copia para o endereço reg e exclui o registro do topo
...
bool excluirElementoPilha(PILHATRIPLA* p, REGISTRO* reg, int pilha){
    if (pilha < 1 || pilha > 3) return false;
    if (tamanhoUmaPilha(p, pilha) == 0) return false;
    if(pilha == 1){
      *reg = p->A[p->topo1];
      p->topo1--;
    }
